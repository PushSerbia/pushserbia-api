import { DeepPartial, FindOptionsWhere, InsertResult, Repository } from 'typeorm';
...
export abstract class RepositoryService<T extends { id: string | number }> {
    ...
    async createMany(entities: QueryDeepPartialEntity<T>[]): Promise<InsertResult> {
    ...
    async update(
        ...
        await this.repository.update(criteria as string | number | FindOptionsWhere<T>, entity);
        return typeof criteria === 'string' || typeof criteria === 'number'
            ? this.findOne(criteria)
            : this.repository.findOneBy(criteria as FindOptionsWhere<T>).then(result => {
                if (!result) {
